...
-- Functions
function GetBoundingBox(model, orientation) -- From Devforum post by XAXA. https://devforum.roblox.com/t/216581/8. Alternative to Model:GetCFrame()
	if typeof(model) == "Instance" then model = model:GetDescendants() end
	if not orientation then orientation = CFrame.new() end
	local abs = math.abs
	local inf = math.huge
...
print(sTwoCFrame)print(posX, posY, posZ)
